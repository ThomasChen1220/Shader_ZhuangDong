...
Shader "Lecture1/line_unexpected" {
    Properties {
        ...
        _light_tone_threshold ("light_tone_threshold", Range(1, 10)) = 1
    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _light_tone_threshold)
...
////// Emissive:
                ...
                float _light_tone_threshold_var = UNITY_ACCESS_INSTANCED_PROP( Props, _light_tone_threshold );
                float4 _light_tone_var = UNITY_ACCESS_INSTANCED_PROP( Props, _light_tone );
                float3 emissive = saturate(saturate(round( 0.5*((lerp(_dark_color_var.rgb,_light_color_var.rgb,step((_dark_threshold_var + ( (_pattern_var.r - node_2387) * (_light_threshold_var - _dark_threshold_var) ) / (1.0 - node_2387)),node_4625))+((lerp(float3(1,1,1),saturate(3.0*abs(1.0-2.0*frac((node_9433.r+0.5)+float3(0.0,-1.0/3.0,1.0/3.0)))-1),node_9433.g)*node_9433.b)*clamp(node_4625,(-1.0),0.0))) + (pow(clamp(node_4625,0.0,1.0),_light_tone_threshold_var)*_light_tone_var.rgb)))));
