...
////// Emissive:
                ...
                float3 node_8513 = lerp((clamp(attenuation,(1.0 - _shadow_Intensity_var),1.0)*(_LightColor0.rgb*(((_baseColorMap_var.rgb*_diffuseWarp_var.rgb)*(_metallicMap_var.r*2.0+0.0))+(node_931*(pow(saturate(dot(reflect((lightDirection*(-1.0)),node_4769.rgb),viewDirection)),exp(_specularExp_var.r))*_specularMask_var.r))))),node_8712,(_self_Illumination_Strength_var*_self_Illumination_var.r));
                ...
                float node_6 = pow(1.0-max(0,dot(node_4769.rgb, viewDirection)),_fresnelExp_var);
                float node_7093 = 0.5;
                float2 node_6767 = float2((1.0 - clamp(node_6,0.001,1)),node_7093);
                ...
                float node_9077 = pow(1.0-max(0,dot(node_4769.rgb, viewDirection)),_rimlExp_var);
                float2 node_9544 = float2((1.0 - saturate(clamp(node_9077,0.001,1))),node_7093);
                ...
                float3 node_7037 = ((_fresnelStrength_var*(_fresnelWarpColor_var.r*node_931))+(((_fresnelWarpRim_var.r*_RimStrength_var)*_rimMask_var.r)*node_6576));
                float3 emissive = (node_8513+node_7037);
